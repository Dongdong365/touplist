...
> [!CAUTION]
> 请不要在任何已经严厉声明禁止 使用上传列表 的服务端使用此插件！谢谢！

一个用于 ***RW-HPS*** 的铁锈战争列表公布插件，基于 API 代理实现稳定上列表。  

该插件理论适用于 ***RW-HPS 以及其下游*** 的任何服务端。本插件于2026年7月27日完成v26.07的可用性测试，测试环境内的结果为完全可用。

部分服务端对于RW-HPS的改动可能较大需要自行验证能不能用。比如````publish port 114514```` ,然后去看配置里的``public_port``改没改 来辨别可用性。
> [!WARNING]
> **插件真的能稳定运行吗**
> 
...
>
> 登记IP可能会暴露你的大概位置，如果介意的话请尝试使用其他插件！为此带来的不便，我们正在优化!
...
#### 🔵 情况二：拥有一台有公网IP的云服务器（VPS）将服务端运行至本地——推荐
...
> **注意**：如果你已经拥有一台不错的云服务器，也可以直接将 RW-HPS 安装在云服务器上运行，那样连 FRP 都不需要，直接 `publish ip 云服务器IP`，`publish port 游戏端口`，开启或不开启代理均可，房间直接就是 Y 状态。
